...
| **Crypto-Analyst** (as alpha) | Calibrated regime/ensemble used to gate or trade | Intraday gates "won" then **failed the placebo** (1D + 4h, p=0.18 / 0.36 — one-month mirages); standalone swing **failed breadth** (2 of 7 coins, ETH was luck) | ❌ as standalone / short-horizon alpha. **But see the 2026-07-04 addendum:** the regime signal PASSED as a low-turnover *allocation input* (sleeves beat B&H 6-7/7 OOS) — currently under shadow forward validation. Failed as a predictor, alive as an allocator. |
...
**THE COMPATIBILITY DOCTRINE (permanent, added 2026-07-14).** A calibrated signal is not automatically a tradeable signal. Every signal must define: information horizon · causal availability/lag · expected decay · valid decision frequency · maximum viable turnover · suitable strategy consumers · known incompatible consumers. **A new strategy may consume a signal only after demonstrating that the signal's edge survives the strategy's own fees, turnover, execution delay, and inventory mechanics.** Proof case: the identical regime labels pass in the sleeves (~2 flips/month) and fail in the grid (thousands of fee-paying actions) — the signal was never the variable; the consumer was. Profiles live in the analyst's `MODEL_CARD.md`; this doctrine is why.

**Grid loose end — CLOSED 2026-07-14.** The final run gated on the ANALYST's regime (the classifier that powers the passing sleeves), 4.5yr BTC+ETH: no gate mode beat the ungated grid on either coin (BTC: no-gate +21.1%/Sh 0.29 vs freeze +18.7%, exit −58.9%, hedge −16.4%; ETH: no-gate +6.7% vs all gates negative). The kill shot is the regime split: the no-gate grid's BTC P&L was **range −29.5% / trend +63.3%** — it LOST money in the ranges it exists to harvest and earned only by riding inventory through trends, i.e. it was disguised buy-and-hold paying 9–21% of capital in fees, and it loses to actual buy-and-hold. "Can the analyst improve a grid after fees?" was asked twice (internal ER + analyst regime): **no, both times.** The same classifier information works in the sleeves (2 near-free flips/month) and fails in the grid (thousands of fee-paying actions) — right brain, wrong actuator. ⚰️ **The current grid family is closed. No further grid research is permitted unless a new, pre-registered harvestability hypothesis first satisfies the revival hurdle** (tightened 2026-07-14): beat, out of sample after fees/slippage/selection penalties, ALL FOUR of: the ungated grid, buy-and-hold, passive stablecoin carry, and the live sleeve system. Full analysis + the signal-compatibility lesson: crypto-analyst `STRATEGY_GRAVEYARD.md` and `MODEL_CARD.md` (compatibility profile).
